...
type OKRItem = {
  obj: string;
  krs: { name: string; target: number; current: number; program: string }[];
};

const PERIODS = ["2025-Q1", "2025-Q2", "2025-Q3"] as const;

const KPI_DATA: Record<"co1" | "co2", Record<string, KPI>> = {
  co1: {
    "2025-Q3": {
      revenue: 4200000000,
      cogs: 2350000000,
...
const PROGRAMS: Record<"co1" | "co2", Program[]> = {
  co1: [
    {
      id: "p1",
      name: "ERP Core",
...
const OKR: Record<"co1" | "co2", OKRItem[]> = {
  co1: [
    {
...
function Spark({ values }: { values: number[] }) {
  ...
  React.useEffect(() => {
    ...
    const pad = 4;
    const xs = values.map(
      (_, i) => pad + (i * (w - 2 * pad)) / (values.length - 1)
    );
...
export function UI03ExecutiveCockpit(props: {
  initialCompany?: "co1" | "co2";
  initialPeriod?: (typeof PERIODS)[number];
  compare?: "prev-q" | "prev-y" | "none";
  runwayAlertThreshold?: number; // default 6
}) {
  const [company, setCompany] = React.useState<"co1" | "co2">(
    props.initialCompany ?? "co1"
  );
  const [period, setPeriod] = React.useState<string>(
    props.initialPeriod ?? "2025-Q3"
  );
  const [compare, setCompare] = React.useState<"prev-q" | "prev-y" | "none">(
    props.compare ?? "prev-q"
  );
  ...
  const prevKey =
    compare === "prev-q"
      ? PERIODS[Math.max(0, PERIODS.indexOf(period as any) - 1)]
      : compare === "prev-y"
      ? "2024-Q3"
      : null;
  const prev = (prevKey && (KPI_DATA as any)[company][prevKey]) || null;

  const runway = runwayMonths(now);
  const showAlert = runway < (props.runwayAlertThreshold ?? 6);

  function openKPIDrill(kind: "revenue" | "ebitda" | "runway") {
        ...
        >
          <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
            ...
            <div style={{ fontSize: 14 }}>
              <b>Cảnh báo Runway:</b> Dòng tiền đủ dưới <b>{runway}</b> tháng.
              Khuyến nghị: rà soát OPEX, ưu tiên chương trình ROI cao, hoãn dự
              án rủi ro.
            </div>
          </div>
        </div>
                ...
                {o.krs.map((kr, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        Progress: {pct}% · Program: {kr.program}
                      </div>
                    </div>
                  );
                })}
...
function runTests() {
  ...
  t.push({
    name: "EBITDA calc",
    pass: ebitda(now) === now.revenue - now.cogs - now.opex,
  });
  ...
  t.push({
    name: "OKR has at least 1 objective",
    pass: OKR.co1 && OKR.co1.length > 0,
  });
  alert(
