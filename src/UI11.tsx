...
type Location = "WH-A" | "WH-B";
type Category = "ERP HW" | "Spares" | "Licenses" | "Consumables";
...
type OrderStatus = "New" | "Picking" | "Packed" | "Shipped";
type OrderLine = { skuId: string; qty: number };
type Order = {
  id: string;
  company: Company;
  location: Location;
...
export default function UI11_InventoryWMS() {
  ...
  const [location, setLocation] = React.useState<Location>("WH-A");
  const [category, setCategory] = React.useState<Category | "">("");
  const [search, setSearch] = React.useState("");

  // State
  const [stock, setStock] = React.useState<Stock[]>(STOCK_INIT);
  const [orders, setOrders] = React.useState<Order[]>(ORDERS_INIT);
  const [cycle, setCycle] = React.useState<CycleTask[]>(CYCLE_INIT);
  const [selOrder, setSelOrder] = React.useState<Record<string, boolean>>({});
  const [selRepl, setSelRepl] = React.useState<Record<string, boolean>>({});
  const [selCycle, setSelCycle] = React.useState<Record<string, boolean>>({});
  ...
  const stockView = stock
    ...
    .map((s) => ({ ...s, sku: SKUS.find((k) => k.id === s.skuId)! }))
    .filter(
      (row) =>
  ...
  const suggests: ReplenSuggest[] = stockView
    .map((r) => {
  ...
  // Outbound by lane
  const lane = (st: OrderStatus) => ordersView.filter((o) => o.status === st);

  // Rules
  function pickFillRate(o: Order) {
    let ok = 0,
      total = 0;
    o.lines.forEach((l) => {
      const st = stockView.find((r) => r.skuId === l.skuId);
  ...
  function rulePickOK(o: Order) {
    return pickFillRate(o) >= 95;
  }

  function ruleCycleOK(t: CycleTask) {
  ...
  function ruleReplOK(s: ReplenSuggest) {
...
⚠️ Need review: ${ex.length}${
  ...
  function batchPostCycle() {
    ...
    setSelCycle({});
    alert(`Cycle adjustments
✅ Posted: ${ok.length} [${ok.join(", ") || "—"}]
⚠️ Blocked: ${ex.length}${
      ex.length ? "\n" + ex.map((x) => `• ${x.id}: ${x.reason}`).join("\n") : ""
    }`);

    function nextAdjustment(stIdx: number, newQty: number) {
      setStock((prev) => {
        const arr = [...prev];
        const s = { ...arr[stIdx] };
        s.onHand = newQty;
  ...
  function runTests() {
    ...
    t.push({
      name: "Avail calc sane",
      pass: a.every((v) => Number.isFinite(v)),
    });
    ...
    t.push({
      name: "Pick rule gate",
      pass: aNew ? rulePickOK(aNew) === pickFillRate(aNew) >= 95 : true,
    });
    // 4) Cycle rule: either ≤5% or ≤20m value
    const c = cycleView.find((x) => x.counted != null);
    t.push({
      name: "Cycle rule shape",
      pass: c ? typeof ruleCycleOK(c) === "boolean" : true,
    });
    // 5) KPI invariants
    t.push({
      name: "On-hand ≥ Allocated? (aggregate may not hold per SKU)",
      pass: onHand >= 0 && allocated >= 0,
    });
    alert(
