...
type PeriodM = "2025-07" | "2025-08";
type Currency = "VND" | "USD";
type AccType = "Asset" | "Liability" | "Equity" | "Revenue" | "Expense";
type Segment = "Retail" | "Wholesale" | "Services";
type Account = { id: string; name: string; type: AccType };
type TBRow = {
  company: Company;
  period: PeriodM;
  ccy: Currency;
  accId: string;
  bal: number; // bal = DR - CR in LC
  seg?: Segment; // optional segment
  ic?: boolean; // intercompany P&L marker
};
...
/* ================== FX Rates (base) ================== */
const FX_BASE: Record<PeriodM, { USD: { avg: number; close: number } }> = {
...
function translateRows(
  ...
  companies.forEach((c) => {
    ...
    const trs = rs.map((r) => {
      ...
      return { ...r, ccy: "VND" as Currency, bal: r.bal * rate };
    });
...
function groupByAcc(rows: TBRow[]) {
...
function icEliminateBS(rows: TBRow[]) {
  ...
  const adj: TBRow[] = [
    {
      company: "co1",
      period: rows[0].period as PeriodM,
      ccy: "VND",
      accId: "1300",
      bal: -elimAmt,
    },
...
function icEliminatePL(rows: TBRow[]) {
  ...
  const p = rows[0].period as PeriodM;
  const adj: TBRow[] = [
...
function plFromTotals(totals: { accId: string; bal: number }[]) {
  const rev = -sum(
...
function bsFromTotals(totals: { accId: string; bal: number }[]) {
...
function cashFromTotals(totals: { accId: string; bal: number }[]) {
...
function exportSimplePDF(
  ...
  const obj = (id: number, body: string) => `${id} 0 obj\n${body}\nendobj\n`;
...
/* =============== Main Component =============== */
export default function UI16_Consolidation_v2_PDF() {
  const [period, setPeriod] = React.useState<PeriodM>("2025-08");
  const [tab, setTab] = React.useState<
    "P&L" | "BS" | "CF" | "Variance" | "By Company" | "CTA Roll-forward"
  >("P&L");
  const [seg, setSeg] = React.useState<Segment | "All">("All");
  const [postElim, setPostElim] = React.useState(true);
  const [elimPL, setElimPL] = React.useState(true);
  const [fxAvg, setFxAvg] = React.useState<number>(FX_BASE[period].USD.avg);
  const [fxClose, setFxClose] = React.useState<number>(
  ...
  const CF = (() => {
    ...
    function get(totals: { accId: string; bal: number }[], id: string) {
      return totals.find((x) => x.accId === id)?.bal || 0;
    }
    const curT = cur.totals,
      prvT = prev.totals;
    const Δ = (id: string) => get(curT, id) - get(prvT, id);
    const dAR = Δ("1100"),
      dPre = Δ("1200"),
      dAP = Δ("2000"),
      dAcc = Δ("2100");
  ...
  function runTests() {
    ...
    t.push({
      name: "FX override effective",
      pass: Math.abs(tmp.pl.rev - cur.pl.rev) > 0,
    });
    alert(
  ...
  function openBreakdown(accId: string, title: string) {
  ...
  function exportCSV(kind: "P&L" | "BS" | "CF") {
    ...
    if (kind === "P&L") {
      ...
      cur.totals.forEach(
        (r) =>
          (csv += `${r.accId} ${accBy(r.accId).name},${Math.round(r.bal)}\n`)
      );
  ...
  function exportPDF(kind: "P&L" | "BS" | "CF") {
    ...
    if (kind === "P&L") {
      ...
      if (postElim && elimPL)
        lines.push(`Elim P&L IC: ${fmtVND(-cur.elimICPL)}`);
    } else if (kind === "BS") {
  ...
  function ctaRollForward(period: PeriodM, fxAvg: number, fxClose: number) {
    const prior: PeriodM = period === "2025-08" ? "2025-07" : "2025-08";
    const make = (p: PeriodM) =>
      translateRows(
          ...
          >
            ...
            >
              <label style={{ gridColumn: "span 2" }}>
                ...
                <Select
                  value={period}
                  onChange={(e) => setPeriod(e.target.value as PeriodM)}
                >
                  {(["2025-07", "2025-08"] as PeriodM[]).map((p) => (
          ...
          {tab === "By Company" && (
            ...
            >
              ...
              {(() => {
                const byCo: Record<
                  Company,
                  { rev: number; cogs: number; opex: number; ebitda: number }
                > = {
                  co1: { rev: 0, cogs: 0, opex: 0, ebitda: 0 },
                  co2: { rev: 0, cogs: 0, opex: 0, ebitda: 0 },
                };
                (["co1", "co2"] as Company[]).forEach((c) => {
