...
type Category = "Software" | "Hardware" | "Services" | "Logistics";
type DocSubtype = "CapEx" | "OpEx";
type VendorTier = "Platinum" | "Gold" | "Silver" | "Bronze";
type RiskLevel = "Low" | "Medium" | "High";
type ReqStatus = "Draft" | "RFQ" | "Awarded" | "PO" | "Delivered";
type POStatus = "Pending" | "Approved" | "Rejected" | "Received";
...
type Requisition = {
  ...
  buyer: string;
  status: ReqStatus;
  vendorId?: string; // when awarded
  baseline?: number; // for savings comparison
};

type RFQ = {
  id: string;
  reqId: string;
  vendors: { vendorId: string; quote: number }[];
  status: "Open" | "Closed";
...
type PO = {
  ...
  amount: number;
  status: POStatus;
  approvalSLA: number; // days target
};
...
/* ============ Core Component ============ */
export default function UI05_ProcurementVendorHub() {
  // Filters
  ...
  const [vTier, setVTier] = React.useState<VendorTier | "">("");
  const [vRisk, setVRisk] = React.useState<RiskLevel | "">("");
  const [search, setSearch] = React.useState("");

  // Data state
  const [pos, setPOs] = React.useState<PO[]>(POS_INIT);
  const [drawer, setDrawer] = React.useState<{
    title: string;
    body: React.ReactNode;
  } | null>(null);
  const [selectedPO, setSelectedPO] = React.useState<Record<string, boolean>>(
    {}
  );
  ...
  const poFiltered = poBase.filter((p) => {
    ...
    if (dept && req.dept !== dept) return false;
    if (category && req.category !== category) return false;
    if (buyer && req.buyer !== buyer) return false;
    if (search) {
      const s = search.toLowerCase();
      const hay = [
        p.id,
  ...
  function tierTag(t: VendorTier) {
    const map: Record<VendorTier, [string, string]> = {
            ...
            <div style={{ marginTop: 12 }}>
                  ...
                  {rfq.vendors.map((v, i) => {
                    ...
                    return (
                      <tr key={i}>
                        ...
                        <td>
                          <span style={riskTag(vend.risk)}>{vend.risk}</span>
                        </td>
                      </tr>
                    );
                  })}
              ...
              {rfq.status === "Closed" && rfq.awardedVendorId ? (
                <div style={{ marginTop: 8, fontSize: 13, color: "#374151" }}>
                  Awarded: <b>{vendorName(rfq.awardedVendorId)}</b> at{" "}
      ...
      body: (
        <div>
          ...
          >
            ...
            <div>
              <b>Subtype:</b> {r.docSubtype}
            </div>
            <div>
              <b>Amount:</b> {fmtMoneyVND(p.amount)}
            </div>
            <div>
              <b>Status:</b> {p.status}
            </div>
            <div>
              ...
              <span style={{ ...riskTag(v.risk), marginLeft: 8 }}>
                {v.risk}
              </span>
            </div>
            <div>
  ...
  function batchApprove() {
    ...
    const ruleOk: string[] = [];
    const ruleBlocked: { id: string; reason: string }[] = [];
    ids.forEach((id) => {
          ...
          >
            ...
            >
              <label>
                ...
                <Select
                  value={vTier}
                  onChange={(e) => setVTier(e.target.value as VendorTier | "")}
                >
              ...
              <label>
                ...
                <Select
                  value={vRisk}
                  onChange={(e) => setVRisk(e.target.value as RiskLevel | "")}
                >
                  <option value="">Any</option>
                  <option>Low</option>
