...
type Location = "HCM" | "HN" | "Remote";
type Grade = "G5" | "G6" | "G7" | "M1" | "M2"; // staff to manager
type ReqStatus = "Open" | "Closed";
type CandStage = "Screen" | "Interview" | "Offer";
...
type Separation = { empId: string; company: Company; dept: Dept; date: string };
...
const BANDS: Record<Grade, { min: number; mid: number; max: number }> = {
...
const HC_BUDGET: Record<Company, Record<Dept, number>> = {
...
function avgHeadcount(company: Company, period: Period, dept?: Dept) {
  const { start, end } = qStartEnd(period);
  const a = countActive(company, dept, start);
  const b = countActive(company, dept, end);
  return (a + b) / 2;
}
function separationsIn(company: Company, period: Period, dept?: Dept) {
  const { start, end } = qStartEnd(period);
...
function attritionPct(company: Company, period: Period, dept?: Dept) {
...
/* ====================== Main Component ====================== */
export default function UI08_HRWorkforce() {
  // Filters
  ...
  // State
  const [offers, setOffers] = React.useState<Offer[]>(OFFERS_INIT);
  const [selOffer, setSelOffer] = React.useState<Record<string, boolean>>({});
  ...
  function activeHC(company: Company, dept: Dept) {
    return ROSTER.filter(
      (e) => e.company === company && e.dept === dept && isActiveAt(e, TODAY)
    ).length;
  }
  function withinHCBudget(company: Company, dept: Dept, plus: number) {
  ...
  // Bands
  function bandStatus(grade: Grade, salary: number) {
    const b = BANDS[grade];
    if (!b) return { ok: true, pos: "in" as const };
    if (salary < b.min) return { ok: false, pos: "below" as const };
    if (salary > b.max) return { ok: false, pos: "above" as const };
    return { ok: true, pos: "in" as const };
  }
  ...
  function batchApproveOffers() {
    ...
    const incByDept: Record<Dept, number> = {
      IT: 0,
      Ops: 0,
      Finance: 0,
      HR: 0,
  ...
  function openReq(r: Requisition) {
  ...
  function runTests() {
    ...
    t.push({
      name: "Headcount equals roster",
      pass: kpiHeadcount === roster.length,
    });
    // 3) Band check flags out-of-band
    const testBand = bandStatus("G5", 18_000_000); // below min
    t.push({
      name: "Band below flagged",
      pass: testBand.ok === false && testBand.pos === "below",
    });
    ...
    const before = offers.find((o) => o.id === "OF-5002")?.status;
