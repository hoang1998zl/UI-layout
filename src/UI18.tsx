...
type VatRate = 0 | 0.05 | 0.08 | 0.1 | "exempt";
...
type Txn = {
  ...
  date: string;
  ccy: Currency;
  kind: "Sale" | "Purchase" | "Contractor";
  desc: string;
  base: number; // amount BEFORE VAT in original currency
  vat?: VatRate; // for VAT tabs (sales/purchases)
  inputClaimable?: boolean; // purchase VAT creditable?
  whtRate?: number; // for WHT (0.05 = 5%)
  whtType?: "Services" | "Royalties" | "Interest" | "Others";
};
...
type CITSlice = {
  ...
  opex: number;
  otherIncome: number;
  nondeductibleEntertainment: number; // 50% ND in sample
  depDiff: number; // depreciation difference (+ adds back)
};
...
type Task = {
  ...
  company: Company;
  period: PeriodYM;
  kind: "VAT Return" | "WHT Return" | "CIT Provisional";
  due: string;
  status: "Not started" | "In Progress" | "Filed" | "Paid";
};

/* ================== Constants ================== */
const TODAY = new Date("2025-08-14"); // per project context
const FX: Record<Currency, number> = { VND: 1, USD: 25200 };
...
const CALENDAR_SEED: Omit<Task, "status">[] = [
...
/* ================== Tax Logic ================== */
function vndEq(base: number, ccy: Currency) {
  return base * FX[ccy];
}

function vatSummary(company: Company, period: PeriodYM) {
  ...
  type Bucket = { base: number; vat: number };
...
function whtSummary(company: Company, period: PeriodYM) {
  const rows = TXNS.filter(
    (t) =>
      t.company === company &&
      t.period === period &&
...
function citEstimate(company: Company, period: PeriodYM) {
...
function mkCalendar(company: Company, period: PeriodYM): Task[] {
  const seed = CALENDAR_SEED.filter(
    (t) => t.company === company && t.period === period
  );
  const withStatus: Task[] = seed.map((s) => ({
    ...s,
...
export default function UI18_TaxCompliance() {
  ...
  /* ---------- Actions ---------- */
  function exportCSV(kind: "VAT" | "WHT" | "CIT") {
    let csv = "";
    if (kind === "VAT") {
    ...
    const inputVATAll = sum(
      TXNS.filter(
        (t) =>
          ...
          t.kind === "Purchase" &&
          typeof t.vat === "number"
      ).map((t) => vndEq(t.base, t.ccy) * (t.vat as number))
    );
    const claimRatio =
