 ...
 *  • Batch Approve Quotes (rule: discount ≤ 15% & margin ≥ 25%)
...
type Territory = "North" | "South" | "Central";
type Stage =
  | "Lead"
  | "Qualified"
  | "Proposal"
  ...
  | "Closed Won"
  | "Closed Lost";
type Product = "ERP Suite" | "Field Service" | "HR Cloud" | "Analytics";
type Owner = "Lan Vu" | "Thao Nguyen" | "Minh Pham" | "Trang Do" | "Khoa Bui";
...
type Opportunity = {
  ...
  discountPct: number;
  marginPct: number; // margin% vs net (approx)
  created: string;
  closeDate: string;
  stage: Stage;
  notes?: string;
...
export default function UI09_SalesCRM() {
  // Filters
  ...
  const [ter, setTer] = React.useState<Territory | "">("");
  const [prod, setProd] = React.useState<Product | "">("");
  const [stage, setStage] = React.useState<Stage | "">("");
  const [search, setSearch] = React.useState("");

  // State
  const [ops, setOps] = React.useState<Opportunity[]>(OPS_INIT);
  ...
  // Funnel counts
  const funnel: { stage: Stage; count: number; value: number }[] = (
    [
      "Lead",
      "Qualified",
      "Proposal",
  ...
  // Pipeline lanes
  const lane = (s: Stage) => filtered.filter((o) => o.stage === s);

  // Sorting
  const sorted = [...filtered].sort((a, b) => {
    const order = (v: any) => (v == null ? "" : v);
    let va: any, vb: any;
    switch (sortKey) {
      case "account":
        va = a.account;
  ...
  function batchApproveQuotes() {
    const ids = Object.entries(sel)
      ...
      .map(([id]) => id);
    if (ids.length === 0) {
      alert("Chưa chọn opportunity nào.");
      return;
    }
    const ok: string[] = [];
    ...
    ids.forEach((id) => {
      ...
      if (!o) {
        ex.push({ id, reason: "Not in current view" });
        return;
      }
      const reasons: string[] = [];
      if (!(o.stage === "Proposal" || o.stage === "Negotiation"))
        reasons.push("Stage not eligible");
      if (o.discountPct > 15) reasons.push("Discount > 15%");
      if (o.marginPct < 25) reasons.push("Margin < 25%");
    ...
    setOps((prev) =>
      prev.map((o) => {
        ...
        if (ok.includes(o.id)) {
          if (o.stage === "Proposal")
            return { ...o, stage: "Negotiation", notes: "Auto-approved quote" };
          if (o.stage === "Negotiation")
            return {
              ...o,
              stage: "Closed Won",
              closeDate: toISO(TODAY),
              notes: "Auto-approved & closed",
            };
        }
        return o;
      })
  ...
  function openDrawer(o: Opportunity) {
    setDrawer({
  ...
  function runTests() {
    ...
    const calcW = Math.round(
      sum(filtered.map((o) => o.amount * STAGE_PROB[o.stage]))
    );
    ...
    const sample = {
      ...filtered.find((o) => o.stage === "Negotiation")!,
      discountPct: 20,
      marginPct: 20,
    };
    t.push({
      name: "Rule blocks high disc/low margin",
      pass: sample ? !ruleOK(sample as Opportunity) : true,
    });
    // 4) Funnel covers all filtered
    const fCount = funnel.reduce((a, b) => a + b.count, 0);
    t.push({
      name: "Funnel count equals filtered",
      pass: fCount === filtered.length,
    });
    ...
    setSortDir((d) => (d === "asc" ? "desc" : "asc"));
    t.push({
      name: "Sort mutation sanity",
      pass: typeof before === "string" && typeof after === "string",
    });
    alert(
      ...
      >
        ...
        <div style={{ display: "grid", gap: 16 }}>
          ...
          >
            ...
            >
              ...
              <label>
                ...
                <Select
                  value={ter}
                  onChange={(e) => setTer(e.target.value as Territory | "")}
                >
              ...
              <label>
                ...
                <Select
                  value={prod}
                  onChange={(e) => setProd(e.target.value as Product | "")}
                >
                  <option value="">Any</option>
          ...
          >
            <KPI
              title="Pipeline"
              value={fmtVND(pipeline)}
              hint="open opportunities"
            />
            <KPI
              title="Commit"
              value={fmtVND(commitVal)}
              hint="Negotiation + Won"
            />
            <KPI
              title="Weighted FC"
              value={fmtVND(weighted)}
              hint="∑ amount×prob"
            />
            <KPI
              title="Win rate"
            ...
            <KPI
              title="Avg deal"
              value={fmtVND(Math.round(avgDeal) || 0)}
              hint="open+won"
            />
            <KPI
              title="Avg cycle (won)"
              value={(Math.round(avgCycleWon) || 0) + " d"}
              hint="created→close"
            />
          </div>
  ...
  function ths(key: SortKey, label: string) {
