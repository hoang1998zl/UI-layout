...
const FIN_KPI: Record<Company, Record<Period, FinKPI>> = {
  co1: {
    "2025-Q3": {
      revenue: 4200000000,
      cogs: 2350000000,
...
const VENDORS = [
  { id: "v1", name: "Alpha Ltd", tier: "Gold", risk: "Low" as RiskLevel },
  { id: "v2", name: "Bravo Co", tier: "Silver", risk: "Medium" as RiskLevel },
  { id: "v3", name: "Ceta LLC", tier: "Bronze", risk: "High" as RiskLevel },
  { id: "v4", name: "Delta Inc", tier: "Gold", risk: "Medium" as RiskLevel },
  { id: "v5", name: "Echo JSC", tier: "Platinum", risk: "Low" as RiskLevel },
];
...
type BankAcc = { id: string; name: string; number: string; balance: number };
const BANKS: Record<Company, BankAcc[]> = {
  co1: [
...
const CASH_30D: Record<Company, Record<Period, number[]>> = {
  co1: {
    "2025-Q3": [
...
const FCST_13W: Record<Company, ForecastItem[]> = {
...
function fmtMoneyVND(x: number) {
  return x.toLocaleString("vi-VN") + " ₫";
}
function daysInPeriod(p: Period) {
...
/* ============ Main Component ============ */
export default function UI06_FinanceCash() {
  ...
  const [ap, setAP] = React.useState<AP[]>(AP_OPEN_INIT);
  const [selectedAP, setSelectedAP] = React.useState<Record<string, boolean>>(
    {}
  );
  ...
  // Payment rules
  function vendorRisk(id: string) {
    return VENDORS.find((v) => v.id === id)?.risk || ("Medium" as RiskLevel);
  }
  function dueWithin7(dueISO: string) {
    const d = new Date(dueISO).getTime();
    const diff = Math.ceil((d - today.getTime()) / (1000 * 3600 * 24));
    return diff <= 7; // including overdue
  }
  function ruleOK(inv: AP) {
  ...
  function proposePayments() {
    ...
    ids.forEach((id) => {
      ...
      if (!(risk === "Low" || risk === "Medium"))
        reasons.push("Vendor risk > Medium");
    ...
    alert(`Đề xuất thanh toán
✅ Hợp lệ: ${ok.length} [${ok.join(", ") || "—"}]
⚠️ Cần exception: ${ex.length}${
  ...
  function approvePayments() {
    ...
    setSelectedAP({});
    alert(
      `Đã duyệt & loại khỏi AP Open: ${approved.length} [${approved.join(
        ", "
      )}]`
    );
  }
  ...
  function runTests() {
    ...
    t.push({
      name: "AR aging sums",
      pass: eqWithin(sum(arList), sum(Object.values(arAging))),
    });
    t.push({
      name: "AP aging sums",
      pass: eqWithin(sum(apList), sum(Object.values(apAging))),
    });
    ...
    t.push({ name: "Rule blocks high risk", pass: !ruleOK(sampleHigh as AP) });
    const sampleAmt = { ...apList[0], amount: 900_000_000 };
    t.push({ name: "Rule blocks big amount", pass: !ruleOK(sampleAmt as AP) });
...
function sum(
  arr: number[] | { amount: number }[] | AR[] | AP[] | Record<string, number>
) {
  if (Array.isArray(arr)) {
    if (typeof arr[0] === "number")
      return (arr as number[]).reduce((a, b) => a + b, 0);
    // @ts-ignore
    if (typeof arr[0] === "object" && "amount" in (arr[0] || {}))
      return (arr as any[]).reduce((a, b) => a + (b.amount || 0), 0);
  } else {
...
function buckets(
  ...
  items.forEach((x) => {
    out[x.bucket] = (out[x.bucket] || 0) + x.amount;
  });
  return out as Record<(typeof order)[number], number>;
}
function roll(deltas: number[]) {
  let acc = 0;
  const out: number[] = [];
...
function ARTable({ rows, onOpen }: { rows: AR[]; onOpen: (r: AR) => void }) {
  return (
    <div style={{ overflowX: "auto" }}>
      <table
  ...
  function td(first = false): React.CSSProperties {
    return {
      padding: "10px 12px",
      borderBottom: "1px solid #f3f4f6",
      whiteSpace: first ? "nowrap" : undefined,
...
function APTable({ rows, onOpen }: { rows: AP[]; onOpen: (r: AP) => void }) {
  return (
    <div style={{ overflowX: "auto" }}>
      <table
        style={{ width: "100%", borderCollapse: "separate", borderSpacing: 0 }}
