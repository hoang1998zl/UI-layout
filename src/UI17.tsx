...
type CashEvent = {
  ...
  ccy: Currency;
  amount: number;
  type: "AR Inflow" | "AP Outflow" | "Payroll" | "Tax" | "Other";
};
type Payable = {
  id: string;
  company: Company;
...
function vndEq(amount: number, ccy: Currency) {
  return amount * FX[ccy];
}

/* ================== Main Component ================== */
export default function UI17_Treasury() {
  const [company, setCompany] = React.useState<Company>("co1");
  const [horizon, setHorizon] = React.useState<7 | 14 | 30>(14);
  const [payables, setPayables] = React.useState<Payable[]>(PAYABLES_INIT);
  const [batches, setBatches] = React.useState<PaymentBatch[]>([]);
  const [stmts, setStmts] = React.useState<StatementTxn[]>(STATEMENTS);
  ...
  const glCashSim: {
    ...
    ...stmtsView.map((s) => ({
      ...
      date: s.date,
      amount:
        s.amount + (Math.random() < 0.3 ? (s.amount > 0 ? -5000 : +5000) : 0),
      desc: "GL " + s.desc,
      matched: false,
    })),
  ];
  ...
  function createBatch() {
    ...
    const ccy: Currency = items.every((i) => i.ccy === items[0].ccy)
      ? items[0].ccy
      : "VND";
  ...
  function approveBatch(b: PaymentBatch) {
    if (b.status !== "Draft") {
  ...
  function releaseBatch(b: PaymentBatch) {
  ...
  function exportBatchCSV(b: PaymentBatch) {
  ...
  function autoSuggestMatch() {
    const withinDays = (d1: string, d2: string) =>
  ...
  function bulkMatchSelected(ids: string[]) {
    setStmts((prev) =>
