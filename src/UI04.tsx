...
type RiskType = "Vendor" | "Process" | "Security" | "Compliance" | "Financial";
type ControlStatus = "Pass" | "Fail" | "Partial";
type RiskStatus = "Open" | "Mitigation" | "Closed";

type Control = { name: string; status: ControlStatus };
type Risk = {
  id: string;
  title: string;
  company: Company;
...
function level(r: Risk): "Low" | "Medium" | "High" | "Extreme" {
  const s = score(r);
  if (s <= 6) return "Low";
...
function isOverdue(r: Risk) {
  return (
    new Date(r.nextReview).getTime() < TODAY.getTime() && r.status !== "Closed"
  );
}
function avgMttr(items: Risk[]): number {
...
const tagLevel = (lv: "Low" | "Medium" | "High" | "Extreme") =>
  lv === "Extreme"
...
export default function UI04_RiskCompliance() {
  // Filters
  ...
  const [rtype, setRtype] = React.useState<RiskType | "">("");
  ...
  function openRiskDrawer(r: Risk) {
    const lv = level(r);
  ...
  function runTests() {
    ...
    const baseNow = RISKS.filter(
      (r) =>
        r.company === company && r.period === period && r.status !== "Closed"
    );
    ...
    t.push({
      name: "Has overdue (sanity)",
      pass: typeof hasOverdue === "boolean",
    });
          ...
          >
            ...
            >
              ...
              <label>
                ...
                <Select
                  value={rtype}
                  onChange={(e) => setRtype(e.target.value as RiskType | "")}
                >
                  <option value="">All</option>
          ...
          >
            ...
            {cellFilter && (
              <div style={{ marginTop: 8, fontSize: 12 }}>
                Filtered by cell: Impact <b>{cellFilter.impact}</b> × Likelihood{" "}
                <b>{cellFilter.likelihood}</b>.{" "}
                <Button onClick={() => setCellFilter(null)}>Clear</Button>
              </div>
            )}
          </div>
