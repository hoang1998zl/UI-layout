...
type ScenarioName = "Base" | "Upside" | "Downside" | string;
type AccountType = "Revenue" | "COGS" | "Opex" | "Capex";
type AccountRow = {
  id: string;
  name: string;
  type: AccountType;
...
type Drivers = {
  ...
  unitCost: number; // VND per unit
  volume: Record<Month, number>; // units per month
  headcount: number; // people
  salary: number; // VND per person per month
};
type Scenario = {
  name: ScenarioName;
  company: Company;
  ...
  year: number;
  drivers: Drivers;
  accounts: AccountRow[]; // Revenue/COGS derived; Opex editable
  locked: boolean;
  createdAt: string;
};

const DEPTS: Dept[] = ["Finance", "IT", "Ops", "HR"];
...
function sumMonths(m: Record<Month, number>) {
  return MONTHS.reduce((s, k) => s + (m[k] || 0), 0);
}
function zeros(): Record<Month, number> {
  const o: any = {};
  MONTHS.forEach((m) => (o[m] = 0));
  return o;
}
function mapMonths(fn: (m: Month, i: number) => number) {
  const o: any = {};
  MONTHS.forEach((mm, i) => (o[mm] = fn(mm, i)));
  return o as Record<Month, number>;
}
function addMonths(a: Record<Month, number>, b: Record<Month, number>) {
  const o: any = {};
  MONTHS.forEach((m) => (o[m] = (a[m] || 0) + (b[m] || 0)));
  return o;
}
function mulMonths(a: Record<Month, number>, k: number) {
  const o: any = {};
  MONTHS.forEach((m) => (o[m] = (a[m] || 0) * k));
  return o;
}
function eqWithin(a: number, b: number, tol = 1) {
...
function spreadTotalToMonths(
  total: number,
  pattern: SpreadKey
): Record<Month, number> {
  const w = SPREADS[pattern];
  const out: any = {};
  let acc = 0;
  for (let i = 0; i < 12; i++) {
    const val = Math.round(total * w[i]);
    out[MONTHS[i]] = val;
    acc += val;
  }
...
/* ====================== Seed Data ====================== */
function defaultDrivers(): Drivers {
  return {
    price: 1_200_000,
    unitCost: 700_000,
    volume: mapMonths(
      (_m, i) =>
        [800, 820, 840, 860, 900, 950, 980, 1000, 1100, 1200, 1300, 1400][i]
    ),
    headcount: 18,
    salary: 22_000_000,
  };
...
function buildDerivedAccounts(drv: Drivers): {
  revenue: AccountRow;
  cogs: AccountRow;
  payroll: AccountRow;
} {
  // Revenue = price * volume (per month)
  const revenue = {} as Record<Month, number>;
  const cogs = {} as Record<Month, number>;
  const payroll = {} as Record<Month, number>;
  MONTHS.forEach((m) => {
    revenue[m] = Math.round((drv.price || 0) * (drv.volume[m] || 0));
    cogs[m] = Math.round((drv.unitCost || 0) * (drv.volume[m] || 0));
...
/* ====================== KPI & Math ====================== */
function totals(accounts: AccountRow[]) {
  const by = (t: AccountType) =>
    accounts
      .filter((r) => r.type === t)
...
function variance(cur: number, base: number) {
  const abs = cur - base;
  const pct = base === 0 ? 0 : (abs / base) * 100;
  return { abs, pct };
}
function isBigVar(cur: number, base: number, thresholdPct = 10) {
  const { pct } = variance(cur, base);
  return Math.abs(pct) >= thresholdPct;
}

/* ====================== Main Component ====================== */
export default function UI07_BudgetPlanner() {
  // Filters
  const [company, setCompany] = React.useState<Company>("co1");
  const [dept, setDept] = React.useState<Dept>("IT");

  // Scenario store (local)
  const [scenarios, setScenarios] = React.useState<Record<string, Scenario>>(
    () => {
  ...
  const [curName, setCurName] = React.useState<ScenarioName>("Base");
  const baseScenario = scenarios["Base"];
  const cur = scenarios[curName];

  // UI states
  const [spread, setSpread] = React.useState<SpreadKey>("flat");
  ...
  function recomputeAccounts(s: Scenario) {
  ...
  function updateDrivers(patch: Partial<Drivers>) {
  ...
  function applyVolumeSpread(totalUnits: number) {
    if (cur.locked) return alert("Scenario is locked.");
  ...
  function editOpex(rowId: string, month: Month, val: number) {
    if (cur.locked) return alert("Scenario is locked.");
    const next = clone(cur);
          ...
          <div>
            ...
            <ul style={{ paddingLeft: 18, marginTop: 8 }}>
              {issues.slice(0, 20).map((it, i) => (
                <li key={i}>
                  {it.row} · {PERIOD_LABELS[it.month]}: {fmtVND(it.cur)} vs Base{" "}
          ...
          <table style={{ width: "100%", fontSize: 13, marginTop: 6 }}>
            ...
            <tbody>
              {cur.accounts.map((row, i) => {
                ...
                const bt = sumMonths(baseRow ? baseRow.monthly : zeros());
            ...
            >
              ...
              <label>
                ...
                <Input
                  type="number"
                  value={whatIf}
                  onChange={(e) => setWhatIf(Number(e.target.value) || 0)}
                />
              </label>
            </div>
              ...
              >
                ...
                <tfoot>
                  <tr style={{ fontWeight: 600 }}>
                    ...
                    {MONTHS.map((m) => {
                      const rev = cur.accounts.find((r) => r.id === "rev")!
                        .monthly[m];
                      const cogs = cur.accounts.find((r) => r.id === "cogs")!
                        .monthly[m];
        ...
        <div style={{ display: "grid", gap: 16 }}>
          ...
          >
            ...
            <ul style={{ fontSize: 14, paddingLeft: 18 }}>
              <li>
                Dùng <b>spread</b> (flat/front/back/seasonal) để phân bổ nhanh
                theo mùa vụ.
              </li>
